...
class AnchorText(object):
    ...
    def get_sample_fn(self, text, classifier_fn, use_proba=False):
        true_label = 'Sun Life Stadium'
        #true_label = 'Denver Broncos'
        #true_label = 'Saint Bernadette Soubirous'  #classifier_fn(text)
        ...
        def sample_fn(present, num_samples, compute_labels=True):
            if self.use_unk_distribution:
                ...
                for i, t in enumerate(words):
                    if t == ';':
                        print("SEMICOLON")
                        continue
                    if i<=semi_ind:  # Don't change the context, only the question
                        continue
                    if i in present:
                        continue
            ...
            labels = []
            if compute_labels:
                #if classifier_fn(raw_data) == true_label:
                #    label
                for item in raw_data:  #each modified context gets checked
                    print(item.split(';')[1])
                    labels.append(int(classifier_fn(item)==true_label))
                 #labels = int(classifier_fn(raw_data) == true_label) #.astype(int)
